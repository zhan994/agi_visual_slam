...
void image_callback(const sensor_msgs::ImageConstPtr& image_msg) {
  ...
  if (last_image_time == -1) last_image_time = image_msg->header.stamp.toSec();
  // 检查时间戳是否错乱以及延时过大
...
int main(int argc, char** argv) {
  ...
  if (LOOP_CLOSURE) {
    ...
    string vocabulary_file =
        pkg_path +
        "/../support_files/brief_k10L6.bin";  // 训练好的二进制词袋的路径
    cout << "vocabulary_file" << vocabulary_file << endl;
    posegraph.loadVocabulary(vocabulary_file);  // 加载二进制词袋

    BRIEF_PATTERN_FILE =
        pkg_path +
        "/../support_files/brief_pattern.yml";  // 计算描述子pattern的文件
