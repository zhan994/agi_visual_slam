...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index) {
  ...
  if (DEBUG_IMAGE) {
    ...
    putText(compressed_image, "feature_num:" + to_string(feature_num),
            cv::Point2f(10, 10), CV_FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255));
  ...
  if (DEBUG_IMAGE) {
    ...
    if (ret.size() > 0)
      putText(loop_result, "neighbour score:" + to_string(ret[0].Score),
              cv::Point2f(10, 50), CV_FONT_HERSHEY_SIMPLEX, 0.5,
              cv::Scalar(255));
  }
  // visual loop result
  if (DEBUG_IMAGE) {
      ...
      if (ret[i].Score > 0.015) {
        ...
        if (DEBUG_IMAGE && 0) {
          ...
          putText(tmp_image, "loop score:" + to_string(ret[i].Score),
                  cv::Point2f(10, 50), CV_FONT_HERSHEY_SIMPLEX, 0.4,
                  cv::Scalar(255));
          cv::hconcat(loop_result, tmp_image, loop_result);
        }
      }
...
void PoseGraph::optimize4DoF() {
  while (true) {
    ...
    if (cur_index != -1) {
      ...
      int max_length =
          cur_index + 1;  // 预设最大长度，总之优化帧数不可能超过这么多
      ...
      for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
        ...
        if ((*it)->has_loop) {
          ...
          int connected_index =
              getKeyFrame((*it)->loop_index)
                  ->local_index;  // 得到回环帧在这次优化中的idx
          Vector3d euler_conncected =
              Utility::R2ypr(q_array[connected_index].toRotationMatrix());
          Vector3d relative_t;
          relative_t =
              (*it)->getLoopRelativeT();  // 得到当前帧和回环帧的相对位姿
