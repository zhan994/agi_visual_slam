...
void img_callback(const sensor_msgs::ImageConstPtr& img_msg) {
      ...
      FREQ)  // 保证发给后端的不超过这个频率
  {
    ...
    // 这段时间的频率和预设频率十分接近，就认为这段时间很棒，重启一下，避免delta
    // t太大
  ...
  if (PUB_THIS_FRAME) {
    ...
    if (!init_pub) {
      ...
      pub_img.publish(
          feature_points);  // 前端得到的信息通过这个publisher发布出去
...
int main(int argc, char** argv) {
  ...
  ros::console::set_logger_level(
      ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Info);  // 设置ros log级别
  ...
  // 注册一些publisher
  pub_img = n.advertise<sensor_msgs::PointCloud>(
      "feature", 1000);  // 实际发出去的是 /feature_tracker/feature
